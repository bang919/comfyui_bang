...
class PerspectiveRegionInsertDebug:
    """
    透视图插入节点（调试版） - 将目标图片通过透视变换融合到背景图中
    ...
    def find_quadrilateral_from_mask(self, mask, debug=False):
        ...
        if debug:
            print(f"[DEBUG] 初始逼近点数: {len(approx)}")
        ...
        while len(approx) > 4 and epsilon < 0.1 * cv2.arcLength(largest_contour, True):
            ...
            approx = cv2.approxPolyDP(largest_contour, epsilon, True)
            if debug:
                print(f"[DEBUG] 调整epsilon后点数: {len(approx)}")
    ...
    def execute(self, background_image, background_mask, target_image, 
                rotation_angle, feather_inner_expand, feather_outer_expand, debug_mode):
        try:
            print(f"[DEBUG] 开始执行透视插入节点，调试模式: {debug_mode}")
...
# 节点映射
NODE_CLASS_MAPPINGS = {
    "PerspectiveRegionInsertDebug": PerspectiveRegionInsertDebug
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "PerspectiveRegionInsertDebug": "透视区域插入（调试版） (Perspective Region Insert Debug)"
} 